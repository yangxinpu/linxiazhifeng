...
import { message } from 'antd'; // 可替换为项目UI库的提示组件
import { RequestError } from '../api/type';

// 请求函数类型
type RequestFn<P extends any[], D> = (...params: P) => Promise<D>;

// Hook 配置项
interface UseRequestOptions<P extends any[], D> {
  auto?: boolean;/** 组件挂载时是否自动执行一次请求，默认 false */
  /** 自动执行时的默认参数 */
  defaultParams?: P;
  /** 是否自动弹出错误提示，默认 true */
  showError?: boolean;
...
// Hook 返回值
interface UseRequestResult<P extends any[], D> {
...
export function useRequest<P extends any[], D>(
